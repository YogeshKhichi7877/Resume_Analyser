...
const AuthPage = () => {
  ...
  const API_BASE_URL = 'https://resume-analyser-ch1f.onrender.com';
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Success handling: Save token and redirect
      ...
      // Redirect user to the main app/dashboard after successful auth
      navigate('/dashboard'); // Change '/dashboard' to whatever your main route is
        ...
        <div className="bg-white border-4 border-black p-8 shadow-[12px_12px_0px_0px_rgba(0,0,0,1)] relative z-0 rounded-br-2xl rounded-bl-2xl">
          ...
          {error && (
            <div className="mb-6 bg-red-50 border-l-4 border-red-500 p-4 rounded-r-lg animate-pulse">
              <div className="flex items-start gap-3">
                <AlertTriangle className="w-5 h-5 text-red-600 mt-0.5 flex-shrink-0" />
                <div>
                  <p className="font-bold text-red-800 text-sm">Authentication Error</p>
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <button
              ...
              className={`w-full py-4 bg-gradient-to-r from-blue-600 to-purple-600 text-white font-black text-xl border-4 border-black uppercase tracking-widest flex items-center justify-center gap-3 rounded-xl transition-all
                ${isSubmitting 
                  ? 'opacity-70 cursor-not-allowed shadow-none translate-x-1 translate-y-1' 
                  : 'shadow-[6px_6px_0px_0px_rgba(0,0,0,1)] hover:shadow-none hover:translate-x-1 hover:translate-y-1'
                }`}
            >
              {isLogin ? <LogIn className="w-6 h-6" /> : <UserPlus className="w-6 h-6" />}
              {isSubmitting ? 'Processing...' : (isLogin ? 'Enter App' : 'Create Account')}
            </button>
          </form>
